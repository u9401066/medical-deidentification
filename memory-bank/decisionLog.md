...
| 2026-05-11 | Log 不是除錯垃圾桶，是第二份資料庫 | Production log/error path 不可記錄原始 PHI、檔案路徑、模型輸出片段或前端 stack；改為 safe exception summary、結構化 metadata 與 redacted frontend error report。 |
| 2026-05-11 | 表格下載以 masked structured data 為準 | CSV/XLSX 結果下載需保留原始列欄形狀並套用 masked replacements；`masked_content` 僅作文字/fallback，不能再截斷到 5000 字造成驗收與下載錯誤。 |
| 2026-05-11 | Release 更新必須有備份與真入口 smoke | systemd/env/data/log 更新前先產生 root-only runtime tarball；anonymous LAN 與 password/RBAC production 都需透過 frontend `/api` proxy 路徑 smoke，避免只測 backend 造成假通過。 |
