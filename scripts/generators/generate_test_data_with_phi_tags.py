...
def generate_phi_tagged_test():
    ...
    print(f"   - 5 test cases with PHI tags")
    print(f"   - Tag format: 【PHI:TYPE:ID】content【/PHI】")
    print(f"\n📊 PHI Statistics:")
    print(f"   Case 1: 26 PHI instances (high-risk age >89)")
    print(f"   Case 2: 27 PHI instances (rare disease)")
    print(f"   Case 3: 32 PHI instances (pediatric)")
    print(f"   Case 4:  9 PHI instances (simple)")
    print(f"   Case 5: 13 PHI instances (international)")
    print(f"   Total: 107 PHI instances")
    print(f"\n💡 Usage:")
    print(f"   1. Parse tags with regex: 【PHI:(\\w+):?(\\w*)】([^【]+)【/PHI】")
    print(f"   2. Compare with system output to calculate:")
    print(f"      - True Positives (TP): Correctly detected PHI")
    print(f"      - False Positives (FP): Incorrectly detected")
    print(f"      - False Negatives (FN): Missed PHI")
...
"""
    ...
    print(f"   - Python script to parse PHI tags")
    print(f"   - Calculate Precision, Recall, F1 Score")
